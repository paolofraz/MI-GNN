...
def name_to_dict(run, return_params=False):
    # Fix IMDB
    run = run.replace('IMDB-', 'IMDB_')
    cols = run.split('-')[1:]
    keys, vals = [], []
    for col in cols:
        try:
            ...
            print("Error in fetching ",col)
        keys.append(key)
        vals.append(string_to_python(val))

    if return_params: # used as dir
        # Construct the path to the '0' subfolder.
        base_dir = return_params.split("/")[-1]
        log_path = os.path.join(os.getcwd(), 'results', base_dir, run, '0', 'logging.log')
        ...
        if os.path.isfile(log_path):
            ...
            with open(log_path, 'r') as f:
                log_contents = f.read()
                # Use regular expressions to extract the number that follows the string 'Num parameters:'.
                num_params_match = re.search(r'Num parameters:\s*(\d+)', log_contents)
                if num_params_match:
                    keys.append('params')
                    vals.append(num_params_match.group(1))
...
def agg_runs(dir, metric_best='auto'):
            ...
            if split in os.listdir(dir_seed):
                ...
                best_epoch = \
                    stats_list[
                        ...
                        'epoch']
                print(f"Best {split} epoch: {best_epoch}")
    ...
    for key, value in results.items():
        ...
        if cfg.tensorboard_agg:
            ...
            writer.close()
    # Saves best results based on Best Validation Epoch
    ...
    # Report best VAL results to RAY, adding the TEST score
    report = {}

    # Iterate through the keys in the 'val' dictionary
    for key in results_best['val'].keys():
        if key in {"accuracy", "accuracy_std"}:
            # Special case for some keys
            report[f"test_{key}"] = results_best['test'][key]
            report[f"val_{key}"] = results_best['val'][key]
        else:
            # For other keys, simply copy the values from the 'val' dictionary
            report[key] = results_best['val'][key]

    # Report the best val results to RAY
    print(f"TRIAL ID {session.get_trial_id()} reporting best val results to RAY: \n {report}")
...
def agg_batch(dir, metric_best='auto'):
    ...
    for run in os.listdir(dir):
        if run != 'agg':
            dict_name = name_to_dict(run, return_params=dir)
