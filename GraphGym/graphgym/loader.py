...
from graphgym.models.transform import (edge_nets, ego_nets, path_len,
                                       remove_node_feature)
#from graphgym.contrib.transform.svd import svd_reservoir
...
def load_pyg(name, dataset_dir):
    ...
    if name in ['Cora', 'CiteSeer', 'PubMed']:
        ...
        if name[3:] == 'IMDB':
            ...
            dataset_raw = TUDataset(dataset_dir, name[3:], transform=T.Constant())
        else:
...
def load_dataset():
    ...
    # Load from OGB formatted data
        ...
        if cfg.dataset.name == 'molhiv':
            dataset = PygGraphPropPredDataset(name='ogbg-molhiv', root=dataset_dir)
